...
__global__ void myKernel(Particle* ptr, double dt, int width) {
	...
	if (i < width && j < width) {
		if (ptr[i * width + j].launch) { // 발사되는 동안의 위치 & 속도벡터 계산
			...
			for (int m = 0; m < 15; m++) { // 자취 저장
				for (int k = 0; k < 3; k++) {
					ptr[i * width + j].hist[m][k] = ptr[i * width + j].x[k];
				}
			}

			if (ptr[i * width + j].launchV[1] < 0.0f) {  // 발사 종료 조건
...
void Render() {
	...
	// 픽셀 버퍼를 CUDA 시스템에 매핑하고, 매핑된 GPU 주소(pDevImage)를 구함
	size_t size;
